...
def loadDataSet():
    goods=[]
    with open(r'C:\Users\Administrator\Desktop\商品关联分析Apriori\goods_L.csv',encoding='utf-8') as csvFile:
        csvReader=csv.reader(csvFile)
        for row in csvReader:
            goodsList=row[1].split('_')
...
def calcConf(freqSet, H, supportData, brl, minConf=0.7):
    ...
    for conseq in H:#后件，遍历 H中的所有项集并计算它们的可信度值
        ...
        if conf >= minConf:
            print (freqSet-conseq,'-->',conseq,'conf:',conf,supportData[freqSet-conseq])
...
if __name__=='__main__':
    data=loadDataSet()
    L,supportData=apriori(data,minSupport=0.001)
