...
sc_labels_dic_per
sc_labels={0:'1条跑单县路',1:'2-3条跑单县路',2:'4条以上跑单县路'}
...
reply_ag_labels_dic_per
reply_ag_labels={0:'经纪人运单占比23%以内|依赖程度低',1:'经纪人运单占比72%以上|依赖程度高',2:'经纪人运单占比23-71%|依赖程度中'}
...
yconnect= create_engine('mysql+pymysql://{0}:{1}@{2}:{3}/{4}?charset=utf8'.format(user,passwd,host,port,db))

#yconnect= create_engine('mysql+pymysql://root:password@localhost:3306/databasename?charset=utf8')

pd.io.sql.to_sql(df_merge_1,'rep_op_driver_clusters_result', yconnect, schema='repm', if_exists='append')
