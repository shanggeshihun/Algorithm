...
-- 创建临时有效运单表（剔除取消订单及测试公司，煤炭类型）
drop table if exists repm.temp_not_cancelled
_orders;
set @etl_date='2019-02-25';
create table repm.temp_not_cancelled
_orders as 
select o.* ,
    -- 签收单则记录运输耗费天数
...
-- 聚类样本数据表
repm.rep_op_dri_order_and_login_cluster_samples
repm.rep_op_dri_stocksw_cluster_samples
...
-- birt
select driver_name,sex,certify_state_all,dri_age,age_labels_define,common_company_name,common_load_address,common_upload_address,com_cnt,plate_numbers_cnt,order_pay,volumn_define_final,order_cnt,per_order_lc,recent_order_days,loss_labels_define,order_freq,active_define_final,loadupload_region_cnt,sc_labels_define,recent_login_days,lg_labels_define,sw_labels_define,agent_order_rate,reply_ag_labels_define
from REP_OP_DRIVE_INFO_AND_CLUSTER
where age_labels_define=?
    and volumn_define_final=?
    and loss_labels_define=?
    and active_define_final=?
    and sc_labels_define=?
    and lg_labels_define=?
    and sw_labels_define=?
    and reply_ag_labels_define=?