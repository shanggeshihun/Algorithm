...
#            第j类dataframe
            ...
            xij_max=np.max(df_temp.loc[:,X[i]])
            xij_min=np.min(df_temp.loc[:,X[i]])
#            添加聚类中心点数据标签
            plt.text(xj+0.1,dfDropexegp.loc[xj,X[i]],np.int(dfDropexegp.loc[xj,X[i]]),ha='center', va= 'bottom')
#            添加最值数据标签
            plt.text(xj+0.1,xij_max,np.int(xij_max),ha='center', va= 'bottom')
            plt.text(xj+0.1,xij_min,np.int(xij_min),ha='center', va= 'bottom')
