...
#原始运单相关数据源
df=pd.read_excel(r"C:\Users\dell\Desktop\rfm_login_3.xlsx")
#分析异常记录
# 1 剔除 编码null的记录
...
loss_labels_dic_per
loss_labels={0:'148-406|轻度流失客户',1:'407-781|重度流失客户',2:'1-147|正常客户'}
...
loss_merge_final['loss_labels_define'].value_counts()
loss_merge_final.head(3)
...
active_labels_dic_per
active_labels={0:'0-28',1:'28-110',2:'110-'}
...
#active子聚类
active_dfDropexe_sub=active_dfDropexe[active_dfDropexe.loc[:,'active_label_pred']==0]
...
active_labels_sub={0:'0-9',1:'10-28'}
...
active_merge.head(3)
active_merge['active_labels_define'].value_counts()
active_merge['active_labels_define_sub'].value_counts()

#子类定义结果赋值给新列
active_merge['active_define_final']=active_merge['active_labels_define_sub']
#left join 子类定义结果为空的index
null_index=active_merge[pd.isnull(active_merge['active_labels_define_sub'])].index
#合并母类和子类
for i in null_index:
    active_merge.loc[i,'active_define_final']=active_merge.loc[i,'active_labels_define']
...
active_merge_final.head(3)

active_merge_final['active_define_final'].value_counts()
...
volumn_labels_dic_per
volumn_labels={0:'9.55万内',1:'40万以上',2:'9.55-40万'}
...
#volumn子聚类
volumn_dfDropexe_sub=volumn_dfDropexe[volumn_dfDropexe.loc[:,'volumn_label_pred']==0]
...
volumn_labels_sub={0:'3.48万以内',1:'3.48-9.55万'}
...
volumn_dfDropexe_sub.head(3)

volumn_merge=pd.merge(volumn_dfDropexe,volumn_dfDropexe_sub,left_on='driver_code',right_on='driver_code',how='left')
volumn_merge.head(3)
volumn_merge['volumn_labels_define'].value_counts()
volumn_merge['volumn_labels_define_sub'].value_counts()

#子类定义结果赋值给新列
volumn_merge['volumn_define_final']=volumn_merge['volumn_labels_define_sub']
#left join 子类定义结果为空的index
null_index=volumn_merge[pd.isnull(volumn_merge['volumn_labels_define_sub'])].index
#合并母类和子类
for i in null_index:
    print(volumn_merge.loc[:,'volumn_labels_define'])
    volumn_merge.loc[i,'volumn_define_final']=volumn_merge.loc[i,'volumn_labels_define']
...
sc_labels_dic_per
sc_labels={0:'1条县路',1:'2-3条县路',2:'4条以上县路'}
...
lg_merge_final.head(3)



##################司机搜索查看货源
sw_df=pd.read_excel(r"C:\Users\dell\Desktop\search_watch_3.xlsx")

colList=['user_code','search_days','first_search_days','recent_search_days','watch_days','first_watch_days','recent_watch_days']
#司机编码或者首次搜索距今null的索引
sw_null_index=sw_df.loc[:,['user_code','first_search_days']].index[np.where(np.isnan(sw_df.loc[:,['user_code','first_search_days']]))[0]].drop_duplicates()
...
"""
sw_merge_final=sw_dfDropexe.loc[:,['user_code','search_days','recent_search_days','watch_days','recent_watch_days','sw_labels_define']]
sw_merge_final.head(3)
...
reply_ag_labels_dic_per
reply_ag_labels={0:'占比23%以内|经纪人依赖程度低',1:'占比72%以上|经纪人依赖程度高',2:'占比23-71%|经纪人依赖程度中等'}
...
df_merge=pd.merge(df_copy,loss_merge_final,left_on ='driver_code',right_on = 'driver_code',how ='left')
df_merge=pd.merge(df_merge,age_merge_final,left_on ='driver_code',right_on = 'driver_code',how ='left')
df_merge=pd.merge(df_merge,active_merge_final,left_on ='driver_code',right_on = 'driver_code',how ='left')
df_merge=pd.merge(df_merge,volumn_merge_final,left_on ='driver_code',right_on = 'driver_code',how ='left')
df_merge=pd.merge(df_merge,sc_merge_final,left_on ='driver_code',right_on = 'driver_code',how ='left')
df_merge=pd.merge(df_merge,lg_merge_final,left_on ='driver_code',right_on = 'driver_code',how ='left')
df_merge=pd.merge(df_merge,sw_merge_final,left_on ='driver_code',right_on = 'user_code',how ='left')
df_merge=pd.merge(df_merge,reply_ag_merge_final,left_on ='driver_code',right_on = 'driver_code',how ='left')
