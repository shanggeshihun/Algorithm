...
#清空聚类结果表
def del_query_mysql(h,p,u,pw,d,sql):
...
def class_samples_scatter(theme,dfDropexe,X,label_pred,best_k):
    ...
    for i in range(1,len(X)):
        ...
        #第j类dataframe
            ...
            xij_mean=np.round(np.mean(df_temp.loc[:,X[i]]),2)
            xij_center=np.round(dfDropexegp.loc[xj,X[i]],2)
            #主要聚类特征字典
            clus_indicator_dic_sub[xj]=[xij_min,xij_max,xij_mean]
            #添加聚类中心点数据标签
...
#原始运单相关数据源
rfm_login="select * from rep_op_dri_order_and_login_cluster_samples"
...
for key in loss_clus_indicator_dic.keys():
    for key_sub in loss_clus_indicator_dic[key].keys():
        ...
        loss_labels[key_sub]=str(int(loss_temp[0]))+'-'+str(int(loss_temp[1]))
loss_labels
...
"""
0 78-220 未跑单 37.2%  》 可能流失客户
1 509-941 未跑单 4.26%  》 重度流失客户
2 221-508 未跑单 18.27%  》 轻度流失客户
3 1-77 未跑单 40.23%  》 正常客户
...
age_labels={}
for key in age_clus_indicator_dic.keys():
    for key_sub in age_clus_indicator_dic[key].keys():
        age_temp=age_clus_indicator_dic[key][key_sub]
        age_labels[key_sub]=str(int(age_temp[0]))+'-'+str(int(age_temp[1]))
age_labels
...
for key in active_clus_indicator_dic.keys():
    for key_sub in active_clus_indicator_dic[key].keys():
        ...
        active_labels[key_sub]=str(int(active_temp[0]))+'-'+str(int(active_temp[1]))
active_labels
...
for key in active_clus_indicator_sub_dic.keys():
    for key_sub in active_clus_indicator_sub_dic[key].keys():
        ...
        active_labels_sub[key_sub]=str(int(active_temp[0]))+'-'+str(int(active_temp[1]))
active_labels_sub
...
for key in volumn_clus_indicator_dic.keys():
    for key_sub in volumn_clus_indicator_dic[key].keys():
        ...
        volumn_labels[key_sub]=str(int(volumn_temp[0]))+'-'+str(int(volumn_temp[1]))
volumn_labels
...
for key in volumn_clus_indicator_sub_dic.keys():
    for key_sub in volumn_clus_indicator_sub_dic[key].keys():
        ...
        volumn_labels_sub[key_sub]=str(int(volumn_temp[0]))+'-'+str(int(volumn_temp[1]))
...
sc_merge_final['sc_labels_define'].value_counts()
"""
...
#登陆次数与首次登录时长、最近登录时长散点图检测异常值(多变量聚类，手动定义类别标签)
lg_df=df_copy.copy()
...
#司机搜索查看货源
search_watch="select * from rep_op_dri_stocksw_cluster_samples"
...
data_dt=datetime.date.strftime(yesterday,'%Y-%m-%d')

df_merge_1.insert(0,'data_dt',data_dt)

#清空聚类结果表
del_query='delete from rep_op_driver_clusters_result'
del_query_mysql(h,p,u,pw,d,del_query)
